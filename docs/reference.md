...
> **對話進行中設定 `game.message`** 不會立刻顯示——訊息會暫存，等對話結束回到地圖後才自動出現。
...
設定後**立刻**播放對話；玩家按 **Space（空白鍵）** 或畫面上的「下一步」逐句推進，最後一句結束後自動回到地圖。

| 欄位 | 說明 |
|------|------|
| `speaker` | 說話者名稱（空字串 `""` = 不顯示名稱欄） |
| `text` | 對話內容 |

**進階：對話結束後執行動作**

`game.dialogue` 本身沒有 callback，若需要「對話結束後再做某件事」，改用 `showDialogue(lines, callback)`：

```js
showDialogue(
  [
    { speaker: "守衛", text: "憑什麼進去？" },
    { speaker: "守衛", text: "……算了，進去吧。" }
  ],
  function() {
    // 對話結束後才執行
    game.setTile(3, 5, 0);         // 把門格改為空地
    game.message = "守衛讓開了！";
  }
);
```
